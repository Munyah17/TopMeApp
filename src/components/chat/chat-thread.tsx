  ...
  return (
    <div>
      <div className="topbar">
        ...
        <div className="ibadge round" style={{ width: 38, height: 38, background: "var(--navy)", color: "#fff", fontWeight: 700, fontSize: 12 }}>
      ...
      <div className="px content-narrow" style={{ paddingTop: 10, paddingBottom: 90 }}>
        ...
        {messages.map((m) => {
          ...
          if (m.kind === "image" && m.image_url) {
            return (
              <div key={m.id} style={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start", marginBottom: 12 }}>
                <div style={{ maxWidth: 220, borderRadius: 16, overflow: "hidden", border: "1px solid var(--border)" }}>
          ...
          return (
            <div key={m.id} style={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start", marginBottom: 10 }}>
              ...
              >
                {m.body}
                <div style={{ fontSize: 10, marginTop: 4, opacity: 0.7, textAlign: "right" }}>{timeOf(m.created_at)}</div>
              </div>
            </div>
          );
        })}
      ...
      <div className="content-narrow" style={{ position: "sticky", bottom: 0, background: "var(--bg)", paddingTop: 8, paddingBottom: 8 }}>
        <div className="px">
          <div className="row gap-2" style={{ alignItems: "center" }}>
            ...
            <button className="backbtn tap" style={{ flexShrink: 0 }} disabled={uploading} onClick={() => fileRef.current?.click()} title="Send a photo">
              <Icon name="image" size={17} stroke={1.8} />
            </button>
            <button className="backbtn tap" style={{ flexShrink: 0, color: "var(--green)" }} onClick={() => setMoneySheet("transfer")} title="Send money or a red packet">
            ...
            <input
              ...
              style={{ flex: 1 }}
            />
            <button className="btn btn-primary tap" style={{ width: 44, height: 44, padding: 0, flexShrink: 0, borderRadius: 14 }} disabled={sending || !text.trim()} onClick={submitText}>
